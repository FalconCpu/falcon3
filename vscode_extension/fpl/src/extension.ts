...
let semanticTokensProvider: FplSemanticTokensProvider;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Register semantic tokens provider
   ...
   const semanticTokensProviderDisposable = vscode.languages.registerDocumentSemanticTokensProvider(
    'fpl', 
...
function compileAndGenerateSymbols(workingDir: string, document: vscode.TextDocument, diagnostics: vscode.DiagnosticCollection) {
    ...
    const errorMap = new Map<string, vscode.Diagnostic[]>();
    const linkerErrors: string[] = [];
    ...
    if (linkerErrors.length > 0) {
      const linkerDiagnostics = linkerErrors.map(error => 
...
function provideHoverInfo(document: vscode.TextDocument, position: vscode.Position): vscode.Hover | undefined {
  ...
  const symbol = symbolDatabase.symbols.find((s: Symbol) => {
...
function provideDefinition(document: vscode.TextDocument, position: vscode.Position): vscode.Location | undefined {
  ...
  const symbol = symbolDatabase.symbols.find((s: Symbol) => {
    ...
    return s.references.some(ref => 
      ref.filename === currentFile && ref.line === position.line + 1
    ) || (s.definition.filename === currentFile && s.definition.line === position.line + 1);
  });
...
class FplSemanticTokensProvider implements vscode.DocumentSemanticTokensProvider {
  private _onDidChangeSemanticTokens: vscode.EventEmitter<void> = new vscode.EventEmitter<void>();
  public readonly onDidChangeSemanticTokens: vscode.Event<void> = this._onDidChangeSemanticTokens.event;
  
  private _documentTokens = new Map<string, vscode.SemanticTokens>();
  ...
  public handleDocumentChange(document: vscode.TextDocument, changes: readonly vscode.TextDocumentContentChangeEvent[]): void {
...
private applyChangesToCachedTokens(
  cachedTokens: vscode.SemanticTokens, 
  changes: readonly vscode.TextDocumentContentChangeEvent[]
): vscode.SemanticTokens | null {
  
  const data = Array.from(cachedTokens.data);
  const tokens: Array<{line: number, char: number, length: number, tokenType: number, tokenModifiers: number}> = [];
  ...
  provideDocumentSemanticTokens(document: vscode.TextDocument): vscode.ProviderResult<vscode.SemanticTokens> {
    ...
    for (const symbol of symbolDatabase.symbols) {
      ...
      if (symbol.definition.filename === currentFile && symbol.definition.line > 0) {
        tokensBuilder.push(
          symbol.definition.line - 1,
          symbol.definition.column - 1,
          symbol.name.length,
          tokenType,
          tokenModifiers
        );
  ...
  private mapSymbolKindToTokenType(kind: string): number {
  ...
  private getTokenModifiers(symbol: any): number {
    let modifiers = 0;
  ...
  private encodeTokens(tokens: SemanticToken[]): vscode.SemanticTokens {
    const tokensBuilder = new vscode.SemanticTokensBuilder(legend);
