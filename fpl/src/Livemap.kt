...
class Livemap(private val func: Function) {
    private val numRows = func.code.size
    ...
    private fun gen() {
        for(instr in func.code) {
            val writes = instr.getDef()
            if (writes!=null)
                kill[instr.index][writes.index]=true
            for(reg in instr.getUse())
    ...
    private fun propagate() {
        ...
        do {
            madeChanges = false
            for (instr in func.code.asReversed()) {
                when (instr) {
                    is InstrRet -> {}
                    is InstrJump -> {
    ...
    fun dump() {
        ...
        for(y in 0..<numRows) {
            print("%-30s".format(func.code[y].toString()))
            for (x in 0..<numCols) {
                val l = live[y][x]
                val k = kill[y][x]
